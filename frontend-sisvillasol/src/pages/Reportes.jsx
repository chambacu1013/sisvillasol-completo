...
function Reportes() {
    ...
    // 2. CARGA DINÁMICA (Cosas que SÍ cambian cuando eliges el año)
    useEffect(() => {
    ...
    // --- CÁLCULO Y CONVERSIÓN DE PRODUCCIÓN ANUAL (KG A TONELADAS) ---
    const totalKilosAnual = ventas.reduce((acumulador, venta) => acumulador + Number(venta.kilos_vendidos || 0), 0);

    const formatearProduccion = (kilos) => {
        if (kilos >= 1000) {
            return `${(kilos / 1000).toFixed(2)} Ton`;
        }
        return `${kilos.toLocaleString()} Kg`;
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try { 
                    ...
                    Swal.fire({
                        icon: 'success', title: 'Eliminado', text: 'El registro ha sido borrado.', timer: 1500, showConfirmButton: false
                    });
    ...
    const listaAnios = [];
    for (let i = anioInicial; i <= anioActual; i++) { listaAnios.push(i); }
    ...
    return (
       <Box sx={{ px: 0, py: 3 }}> 
            ...
            <Grid container spacing={2} sx={{ mb: 4 }}>
                ...
                {/* --- NUEVA TARJETA: PRODUCCIÓN ANUAL EN TONELADAS --- */}
                <Grid item xs={12} sm={6} md={2}>
                    <Card sx={{ bgcolor: '#f3e5f5', borderLeft: '4px solid #9c27b0', height: '100%' }}>
                        <CardContent sx={{ p: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                <ScaleIcon sx={{ color: '#9c27b0', fontSize: 20 }} />
                                <Typography variant="caption" fontWeight="bold">Prod. Anual</Typography>
                            </Box>
                            <Typography variant="h6" fontWeight="bold" color="#9c27b0">
                                {formatearProduccion(totalKilosAnual)}
                            </Typography>
                        </CardContent>
                    </Card>
                </Grid>
        ...
        {/* --- SECCIÓN UNIFICADA: TORTAS Y BARRAS --- */}
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 8, mt: 4 }}>
                ...
                <Box sx={{ flex: '1 1 calc(25% - 4px)', minWidth: 0, '@media (max-width: 900px)': { flex: '1 1 100%' } }}>
                    <Paper sx={{ p: 1, borderRadius: 2, boxShadow: 3, height: 500, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={dataTortas.cultivos} cx="50%" cy="45%" labelLine={false} outerRadius={110} fill="#8884d8" dataKey="value" stroke="none">
                ...
                <Box sx={{ flex: '1 1 calc(25% - 4px)', minWidth: 0, '@media (max-width: 900px)': { flex: '1 1 100%' } }}>
                    <Paper sx={{ p: 1, borderRadius: 2, boxShadow: 3, height: 500, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#555', mb: 1 }}>💸 Inversión (Clic para detalles)</Typography>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={dataTortas.gastos} cx="50%" cy="45%" innerRadius={50} outerRadius={110} dataKey="value" stroke="none" onClick={handleClickTortaGastos} style={{ cursor: 'pointer', outline: 'none' }}>
                ...
                <Box sx={{ flex: '1 1 calc(50% - 4px)', minWidth: 0, '@media (max-width: 900px)': { flex: '1 1 100%' } }}>
                    <Paper sx={{ p: 1, borderRadius: 2, boxShadow: 3, height: 500, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={dataKilos} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                <XAxis dataKey="nombre_lote" angle={-45} textAnchor="end" interval={0} height={60} tick={{fontSize: 10}} />
