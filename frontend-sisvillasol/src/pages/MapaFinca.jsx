...
import { generarPDFHistorialLote } from "../services/historialLoteService";
...
const MapaFinca = () => {
  ...
  const centroFinca = [7.1473, -72.669];
  ...
  const handleClickDescripcion = (event, descripcion) => {
    ...
    setDescripcionActiva(
      descripcion || "Sin detalles adicionales registrados.",
    );
  };
  ...
  const getIconoActividad = (nombre) => {
    ...
    if (n.includes("riego") || n.includes("fertiliza"))
  ...
  //generar pdf del historial de labores del lote seleccionado
  const handleGenerarPDF = async () => {
    if (!loteSeleccionado) {
      Swal.fire(
        "Atención",
        "Debes seleccionar un lote para generar el historial.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      const blob = await generarPDFHistorialLote(loteSeleccionado.id_lote);
  ...
  const handleGuardarEstado = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "¡Actualizado!",
        text: "El estado del lote ha sido modificado.",
        timer: 2000,
          ...
          <FormControlLabel
            ...
            label={
              <Typography
                sx={{
                  fontWeight: "bold",
                  color: verMapaDiseñado ? "#2e7d32" : "#1976d2",
                }}
              >
                {verMapaDiseñado
                  ? "Ver Mapa Satelital (Interactivo)"
                  : "Ver Mapa Diseñado (Gráfico)"}
              </Typography>
            }
          />
        </Box>
              ...
              {lotes.map((lote) => {
                ...
                return (
                  <Marker
                    ...
                    position={[lat, lng]}
                    icon={
                      lote.estado_sanitario === "ALERTA" ? redIcon : greenIcon
                    }
          ...
          >
            ...
            >
              <Box>
                ...
                >
                  Etapa Actual:{" "}
                  <b>{loteSeleccionado.nombre_estado || "Desconocida"}</b>
                </Typography>
                <Typography
                  variant="caption"
              ...
              >
                <Chip
                  label={
                    loteSeleccionado.estado_sanitario === "ALERTA"
                      ? "🔴 ALERTA"
                      : "🟢 ÓPTIMO"
                  }
                  color={
                ...
                <Button
                  ...
                  onClick={() => {
                    setClasificacionFiltro(
                      loteSeleccionado.estado_sanitario || "OPTIMO",
                    );
                    ...
                    {historialLote.length > 0 ? (
                      historialLote.map((fila) => (
                        <TableRow key={fila.id_tarea} hover>
                          <TableCell>
                            {new Date(fila.fecha_ejecucion).toLocaleDateString(
                              "es-CO",
                              { timeZone: "UTC" },
                            )}
                          </TableCell>
                          <TableCell
                            onClick={(e) =>
                              handleClickDescripcion(e, fila.descripcion)
                            }
                            sx={{
                              cursor: "pointer",
